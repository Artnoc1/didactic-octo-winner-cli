...
const getProblem = (node) =>
  node[_missing]
    ? `missing: ${node.pkgid}, required by ${node[_missing]}`
    : node[_invalid]
      ? `invalid: ${node.pkgid} ${node.path}`
      : node.extraneous
        ? `extraneous: ${node.pkgid} ${node.path}`
        : ''
...
const getJsonOutputItem = (node) => {
  ...
  // special formatting for top-level package name
  const hasPackageJson = Object.keys(node.package).length
  if (node.isRoot && hasPackageJson) {
    item.name = node.package.name || node.name || node.path || node.realpath
  }
  ...
  if (node[_include] && node[_parent]) {
    if (!node[_parent].dependencies) {
      node[_parent].dependencies = {}
    }

    node[_parent].dependencies[node.name] = item
  }
...
const ls = async (args) => {
    ...
    getChildren (node, nodeResult) {
      ...
      return (!(node instanceof Arborist.Node) || node[_depth] > depth)
        ...
        : [...node.edgesOut.values()]
          ...
          // TODO: turn this into a sep method like edgesToNodes
          .map(i => {
            if (seen.has(i)) {
              i = {
                name: i.name,
  ...
  if ((json && !Object.keys(result.dependencies).length) || !(result.nodes && result.nodes.length)) {
    if (!json)
      result.nodes = ['(empty)']
  ...
  // TODO: Move this parseable output dance to its own method
  if (parseable) {
    let out = ''
